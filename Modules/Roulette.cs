...
namespace Discord_BOT.Modules
{
    public class Roulette : ModuleBase<SocketCommandContext>
    {
        ...
        {
            var account = UserAccounts.GetAccount(Context.User);
            TimeSpan requiredTime = new TimeSpan(0, 0, Int32.Parse(Config.bot.timeroulette));

            if (Misc.WasRequiredTimeElapsed(account.lastRoulette, requiredTime))
            {
                ...
                UserAccounts.AddCash(account, prize.Cash);
                UserAccounts.AddXP(account, prize.XP);
